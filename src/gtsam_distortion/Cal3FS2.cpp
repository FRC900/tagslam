...
Cal3FS2::Cal3FS2(const gtsam::Vector &v):
  fx_(v[0]), fy_(v[1]), u0_(v[2]), v0_(v[3]), k1_(v[4]), k2_(v[5]), k3_(v[6]), k4_(v[7]){}
...
static gtsam::Matrix28 D2dcalibration(double /*x*/, double /*y*/,
  ...
  //        k1              k2            k3          k4
  DR2 <<
    x_r * theta3, x_r * theta5, x_r *theta7, x_r * theta9, // du/d
...
/* ************************************************************************* */
static gtsam::Matrix2 D2dintrinsic(double r, double r2,
                                   double x_r, double y_r,
                                   double theta_d, double theta,
                                   double theta3, double theta5,
                                   double theta7,
                                   double k1, double k2, double k3, double k4,
                                   const gtsam::Matrix2& DK) {

  const double theta_d_r = (r < 1e-9) ? 1.0 : (theta_d/r);
  const double dtheta_d_dtheta = 1 + theta * (k1 * 3 * theta + k2 * 5 * theta3 + k3 * 7 * theta5 + k4 * 9 * theta7);
  ...
  DR << //            dx                      dy
    theta_d_r + x_r*x_r * dthetad_r_dr, x_r*y_r * dthetad_r_dr, // du
...
gtsam::Point2 Cal3FS2::uncalibrate(const gtsam::Point2& p,
  ...
  if (H1)
    *H1 = D2dcalibration(gp.x, gp.y, gp.xp, gp.yp, gp.x_r, gp.y_r,
                         gp.theta3, gp.theta5, gp.theta7, gp.theta9, DK);

  // Derivative for points
  if (H2)
    *H2 = D2dintrinsic(gp.r, gp.rr, gp.x_r, gp.y_r, gp.theta_d,
                       gp.theta, gp.theta3, gp.theta5, gp.theta7,
                       k1_, k2_, k3_, k4_,
                       DK);
...
/* ************************************************************************* */
gtsam::Point2 Cal3FS2::calibrate(const gtsam::Point2& pi, const double tol) const {
  // map from u,v -> x, y
  ...
  if ( iteration >= maxIterations )
    throw std::runtime_error("Cal3FS2::calibrate fails to converge. need a better initialization");
...
gtsam::Matrix28 Cal3FS2::D2d_calibration(const gtsam::Point2& p) const {
  ...
  // Derivative for calibration
  return (D2dcalibration(gp.x, gp.y, gp.xp, gp.yp, gp.x_r, gp.y_r,
                         gp.theta3, gp.theta5, gp.theta7, gp.theta9, DK));
}
