...
/* ************************************************************************* */
static Eigen::Matrix<double,2,12>
D2dcalibration(double x,   double y,
               double xp,  double yp,
               double x2,  double y2, double xy,
               double R,   double R2, double R3,
               double num, double deninv,
  ...
  DR2 <<
    ...
    R+2*y2,  2*xy, ydinv*R, ydinv*R2, ydinv*R3,  yf*R, yf*R2, yf*R3; // v
...
/* ************************************************************************* */
static gtsam::Matrix2 D2dintrinsic(double x,   double y,
                                   double x2,  double y2,
                                   double xy,
                                   double R,   double R2,
                                   double fR,  double num, double den, 
                                   double deninv,
                                   double p1, double p2,
  ...
  DR << //            dx                      dy
    fR + 2*dfdR*x2 + 2*p1y + 6*p2x,    2*xy*dfdR + 2*p1x + 2*p2y, //du
    2*xy*dfdR      + 2*p1x + 2*p2y,    fR + 2*dfdR*y2 + 6*p1y + 2*p2x; //dv


  return DK * DR;
}

struct GeometricParams3DS2 {
  GeometricParams3DS2(double xa, double ya,
                  double p1, double p2,
                  double k1, double k2, double k3,
                  double k4, double k5, double k6) : x(xa), y(ya) {
#if 0    
    std::cout << "k:  "
              << k1 << " " << k2 << " " << k3 << " "
              << k4 << " " << k5 << " " << k6 << std::endl;
...
gtsam::Point2 Cal3DS3::uncalibrateNoIntrinsics(const gtsam::Point2& p) const {
  GeometricParams3DS2 gp(p.x(), p.y(), p1_, p2_, k_[0], k_[1], k_[2], k_[3], k_[4], k_[5]);
...
gtsam::Point2 Cal3DS3::uncalibrate(const gtsam::Point2& p,
  ...
  if (H1)
    *H1 = D2dcalibration(gp.x, gp.y, gp.xp, gp.yp, gp.x2, gp.y2, gp.xy,
                         gp.R, gp.R2, gp.R3, gp.num, gp.deninv, DK);

  // Derivative for points
  if (H2) {
    *H2 = D2dintrinsic(gp.x, gp.y, gp.x2, gp.y2, gp.xy,
                       gp.R, gp.R2, gp.fR, gp.num, gp.den, gp.deninv,
                       p1_, p2_, k_[0], k_[1], k_[2], k_[3], k_[4], k_[5], DK);
  }
...
/* ************************************************************************* */
gtsam::Point2 Cal3DS3::calibrate(const gtsam::Point2& pi, const double tol) const {
  // map from u,v -> x, y
  ...
  if ( iteration >= maxIterations )
    throw std::runtime_error("Cal3DS3::calibrate fails to converge. need a better initialization");
...
gtsam::Matrix2 Cal3DS3::D2d_intrinsic(const gtsam::Point2& p) const {
  ...
  gtsam::Matrix2 DK;
  DK << fx_, 0.0, 0.0, fy_;
  return (D2dintrinsic(gp.x, gp.y, gp.x2, gp.y2, gp.xy,
                       gp.R, gp.R2, gp.fR, gp.num, gp.den, gp.deninv,
                       p1_, p2_, k_[0], k_[1], k_[2], k_[3], k_[4], k_[5], DK));
}
...
Cal3DS3::D2d_calibration(const gtsam::Point2& p) const {
  GeometricParams3DS2 gp(p.x(), p.y(), p1_, p2_, k_[0], k_[1], k_[2], k_[3], k_[4], k_[5]);
  gtsam::Matrix2 DK;
  DK << fx_, 0.0, 0.0, fy_;
  return (D2dcalibration(gp.x, gp.y, gp.xp, gp.yp, gp.x2, gp.y2, gp.xy,
                         gp.R, gp.R2, gp.R3, gp.num, gp.deninv, DK));
}
